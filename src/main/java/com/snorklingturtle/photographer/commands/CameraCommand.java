...
public class CameraCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        boolean hasPhotoSettingPermission = player.hasPermission("camera.command");
        boolean hasPluginSettingPermission = player.hasPermission("camera.admincommand");

        if (hasPhotoSettingPermission || hasPluginSettingPermission) {
            if (args.length == 0 || args[0].equalsIgnoreCase("help")) {
                ...
                if (hasPluginSettingPermission) {
                    player.sendMessage("§e/camera set_color <material_name> <red> <green> <blue> §7- Adds or sets a color of a material.");
                }
                return true;
            }
        }
        ...
        if (args.length == 5 && args[0].equalsIgnoreCase("set_color")) {
            if (hasPluginSettingPermission)
            {
